...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		// Make sure ExecutionHeader not fall behind FinalizedHeader too much, in that case just
		// pause process FinalizedHeader until ExecutionHeader catch up
		fn cross_check_execution_state() -> DispatchResult {
			...
			if latest_execution_state.beacon_slot > 0 {
				ensure!(
		...
		pub(crate) fn process_execution_header_update(
			...
			let latest_execution_state: ExecutionHeaderState = Self::latest_execution_header();
				...
				update.execution_header.block_number > latest_execution_state.block_number,
				Error::<T>::ExecutionHeaderAlreadyImported
			);
			if latest_execution_state.block_number > 0 {
				ensure!(
					update.execution_header.block_number == latest_execution_state.block_number + 1,
			...
			match &update.ancestry_proof {
				...
				None => {
					...
					let state = <FinalizedHeaderBuffer<T>>::get(block_root)
		...
		fn verify_ancestry_proof(
			...
			let state = <FinalizedHeaderBuffer<T>>::get(finalized_block_root)
				.ok_or(Error::<T>::ExpectedFinalizedHeaderNotStored)?;
