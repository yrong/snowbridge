...
use crate::{self as inbound_queue, fixtures::make_create_message};
...
pub fn setup() {
	...
	.unwrap();
	let test_data = make_create_message();
	EthereumBeaconClient::store_execution_header(
		test_data.message.proof.block_hash,
		test_data.execution_header,
		0,
		H256::default(),
	);
}
